...
function GroupedReasons({ reasons }) {
  ...
  return (
    <div className={cardStyle}>
      <h4 className="text-gray-900 dark:text-white font-semibold mb-5">Analisis & Saran</h4>
          ...
          return (
            <div key={group.tag} className="rounded-xl border border-white/8 overflow-hidden">
              ...
              <div className="divide-y divide-white/5">
                {items.map((item, i) => (
                  <div key={i} className="flex items-start gap-3 px-4 py-3 text-sm text-gray-600 dark:text-white/70 leading-relaxed">
...
function SelectField({ label, name, value, onChange, options }) {
  return (
    <div className="flex-1 min-w-[160px]">
      <label className="block text-xs text-gray-500 dark:text-white/60 mb-1.5 font-medium tracking-wide uppercase">{label}</label>
      <select
        name={name}
        value={value}
        onChange={onChange}
        className={`w-full border-2 rounded-xl px-4 py-3 text-sm font-medium
          ...
          ${value === "" ? "bg-gray-100 dark:bg-gray-800/80 border-gray-300 dark:border-gray-600 text-gray-400 dark:text-white/30" : "bg-gray-100 dark:bg-gray-800/80 border-emerald-500 text-gray-900 dark:text-white"}`}
        style={{ colorScheme: "dark" }}
      >
        <option value="" disabled className="bg-white dark:bg-gray-900 text-gray-400 dark:text-white/30">
          — Pilih {label} —
        </option>
        {options.map((opt) => (
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#F6F3EB] via-[#FAF8F3] to-[#F6F3EB] dark:from-black dark:via-gray-900 dark:to-black pt-24 px-4 md:px-8 pb-12 text-gray-900 dark:text-white">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className={cardStyle}>
          ...
          {!allFilled && !error && <p className="text-gray-400 dark:text-white/30 text-xs mt-3 text-right">Lengkapi semua pilihan untuk mengaktifkan tombol rekomendasi</p>}
        </div>
              ...
              <div className={cardStyle}>
                ...
                {!isTumpangsari ? (
                  <div>
                    ...
                    <p className="text-gray-400 dark:text-white/40 text-sm mt-2">per panen</p>
                  </div>
                ) : (
